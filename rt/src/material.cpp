...
simplemat_t::simplemat_t(std::string _name, color_t _kd, color_t _ks, color_t _kr, color_t _kt, double _eta, double _n, bool _is_r, bool _is_t,bool is_tex,image_t* img):
	material_t(_name),kd(_kd),ks(_ks),kr(_kr),kt(_kt),eta(_eta),n(_n),is_reflect(_is_r),is_transmit(_is_t),is_textured(is_tex)
	{ 
		texture = img;
	}
...
bool simplemat_t::get_is_textured(void) const { return is_textured;}
image_t* simplemat_t::get_texture(void) const {
	return texture;
} 
...
// void simplemat_t::load_texture(const char * filename)
// {
// 	bool check = texture.read_bmp_image(filename);
// 	if(!check)
// 		std::cerr<<"bmp image loading failed";
// }