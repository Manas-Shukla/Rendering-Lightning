...
public:
	...
	/// Returns true if surface is textured, else returns false
	virtual bool get_is_textured(void) const = 0;
	virtual image_t* get_texture(void) const = 0;
...
private:
	...
	/// image object for textures
	image_t *texture;

	/// is material textured or not
	bool is_textured;

public:
	/// Constructor.
	simplemat_t(std::string _name, color_t _kd, color_t _ks, color_t _kr, color_t _kt, double _eta, double _n, bool _is_r, bool _is_t, bool _is_textured, image_t *img);
	/// Destructor.
	virtual ~simplemat_t();
	...
	bool get_is_textured(void) const;
	image_t* get_texture(void) const ;
